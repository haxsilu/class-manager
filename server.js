...
/* ============== UI Frame (Dark + mobile responsive) ============== */
...
.top {
  ...
  gap:2rem; margin:1.2rem 0 1.8rem; flex-wrap:wrap;
}
.top .brand{
  font-weight:700; text-decoration:none; color:var(--text);
  padding:.8rem 1.3rem; border-radius:1rem;
  border:1px solid var(--border); background:var(--chip);
  font-size:1.0rem; display:flex; align-items:center; justify-content:center;
  min-width:160px; text-align:center;
}
.top .nav{ display:flex; gap:.8rem; flex-wrap:wrap; justify-content:center; }
.top .nav a{
  all:unset;
  display:inline-flex !important; align-items:center !important; justify-content:center !important;
  min-width:112px; text-align:center;
  padding:.6rem 1rem !important; border-radius:.9rem !important;
...
.card{ border:1px solid var(--border); border-radius:1rem; padding:1.0rem; background:var(--card-bg); box-shadow:0 8px 28px rgba(3,7,18,.35) }
...
footer{ color:#8aa1bb; text-align:center }

/* Mobile */
@media (max-width:900px){
  .top{ gap:1rem }
  .top .nav a{ min-width:auto; padding:.55rem .8rem !important }
  table{ display:block; overflow-x:auto; white-space:nowrap }
  button, a[role="button"]{ width:100%; margin-top:.4rem }
}
</style>
${head}
</head><body>
<main class="container">
  <nav class="top print-hidden">
    ...
    <div class="nav">
      ...
      <a href="/unpaid"><span>Unpaid</span></a>
  ...
  <footer class="print-hidden" style="margin-top:1.2rem">
        ...
        <tbody>
          ${students.map(s=>`
            <tr>
              ...
              <td><a href="/students/${s.id}/qr" role="button" class="muted">QR</a></td>
...
/* Scanner (with sounds + manual attendance) */
app.get('/scanner', (req,res)=>{
  ...
  <section class="card">
    ...
    <!-- Sounds -->
    <audio id="successSound" preload="auto" src="data:audio/mp3;base64,//uQxAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAAACAAACcQCA//////////8AAAABAAACCgAAAwABAAACcQCA//////////8AAAABAAACCgAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"></audio>
    <audio id="errorSound" preload="auto"   src="data:audio/mp3;base64,//uQxAAAAAAAAAAAAAAAAAAAAAAAWGluZwAAAA8AAAACAAACcQCA//////////8AAAABAAACCgAAAwABAAACcQCA//////////8AAAABAAACCgAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA"></audio>
  ...
  <script>
    ...
    function beep(ok){ const id = ok ? 'successSound' : 'errorSound'; const el = document.getElementById(id); try{ el.currentTime = 0; el.play(); }catch(e){} }
    ...
    async function handleScan(raw){
      ...
      try{
        ...
        if(data.ok){
          ...
          showOk(html); addLog(html); setPay(token); beep(true);
        }else{ showErr(data.error || 'Failed to mark attendance'); setPay(null); beep(false); }
      }catch{ showErr('Network error'); setPay(null); beep(false); }
    }

    document.getElementById('btnManual')?.addEventListener('click', async ()=>{
      const phone = (document.getElementById('manualPhone')?.value||'').trim();
      if(!phone){ showErr('Enter a phone number'); beep(false); return; }
      try{
        ...
        if(data.ok){
          const paidTxt = data.paid ? 'Paid' : 'Unpaid';
          const html = data.student.name + ' · ' + data.class + ' · ' + data.date + ' · ' + paidTxt + ' (' + data.month + ')';
          showOk(html); addLog(html); setPay(data.token); beep(true);
        }else{ showErr(data.error || 'Could not mark attendance'); setPay(null); beep(false); }
      }catch{ showErr('Network error'); setPay(null); beep(false); }
    });
...
/* Unpaid Students (for current month) */
app.get('/unpaid', (req,res)=>{
 ...
 LEFT JOIN payments p     ON p.student_id=s.id AND p.class_id=c.id AND p.month=?
  ...
  const body = `
  <section class="card">
    <form method="get" action="/unpaid" style="display:flex;gap:1.2rem;flex-wrap:wrap;align-items:end">
      <label>Month (YYYY-MM) <input name="month" value="${m}" pattern="\\d{4}-\\d{2}" required></label>
      <button type="submit">Filter</button>
      <a class="muted" role="button" href="/finance?month=${encodeURIComponent(m)}">Open Finance</a>
    </form>
    <div style="overflow:auto; margin-top:.8rem">
      <table>
        <thead><tr><th>Student</th><th>Class</th><th>Phone</th><th>Action</th></tr></thead>
        <tbody>${rows || '<tr><td colspan="4" class="small">Everyone is paid for this month.</td></tr>'}</tbody>
      </table>
    </div>
    <p class="small" style="margin-top:.6rem">Currency: ${currency}. Month: ${m}</p>
  </section>`;
...
app.listen(PORT, ()=>console.log(`Class Manager running on ${BASE} (DB: ${DB_PATH})`));
